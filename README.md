...
- **num_node.txt**  
  ...
  3行目：土質分類数(nc)，ダミー(地表流解析のパラメータ)   
  4行目：土地利用分類数(nlu: 現状4 or 0)，根の深さ(rtd)    
- **category.txt**   
...
- **landuse.txt**  
  土地利用のデータ．要素数（nx-1)×(ny-1）分のデータが並ぶ   
  
  考慮しない場合はnum_node.txtのnlu=0にする   
- **parameter_land.txt**
  土地利用データのパラメータ   
  粘着力cとして与えて調整，すべらないところは十分大きな値に   
  1: 調整なし, 2: 人工物（c非常に大）, 3: 木・竹など, 4: 草のあるところ   
